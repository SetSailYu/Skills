...
description: 扫描前端 / 后端 / 全栈 / Monorepo 项目并生成 VitePress 文档站，输出稳定的信息架构（IA）；UI 层必须接入 ui-ux-pro-max；流程、Checklist 与输出长度遵循 aiinfo-memory-bank；优先使用 Code-Index-MCP 增强架构理解；工程图通过 diagram-mcp-svg 优先生成 SVG（失败回退 PNG） 并按统一规则在 VitePress 中引用。
---
...
- （SVG 失败回退）`docs/public/diagrams/architecture/architecture-system-overview.png`
...
### 8.4 文档引用规范（强制）
VitePress 文档中 **优先使用 SVG** 文件引用（禁止以内嵌长图代码块作为最终交付）。若 SVG 不存在（例如导出失败已回退 PNG），则引用同名 PNG：
...
## 9. 画图策略（MCP 优先，SVG 优先、失败回退 PNG，强制）
...
- 图资产具备 `svg + src`（默认）或 `png + src`（SVG 失败回退）配对文件
