...
description: 扫描前端 / 后端 / 全栈 / Monorepo 项目并生成 VitePress 文档站，输出稳定的信息架构（IA）；UI 层必须接入 ui-ux-pro-max；流程、Checklist 与输出长度遵循 aiinfo-memory-bank；优先使用 Code-Index-MCP 增强架构理解；工程图通过 diagram-mcp-svg 生成图资产（SVG 优先，PNG 等价）并按统一规则在 VitePress 中引用。
---

# VitePress 文档生成与项目扫描 Skill（MCP + SVG 优先画图增强版）
...
## 3. 协同 Skill 契约（统一规则：SVG 优先，PNG 等价）
- **ui-ux-pro-max**：所有 UI/UX 决策必须遵循
- **aiinfo-memory-bank**：Checklist、Output Verbosity、会话节奏与 `.AIINFO` 写入
- **diagram-mcp-svg**：负责生成 图像资产（SVG 优先，PNG 等价）资产与源描述；VitePress 图目录/命名/引用规则由本 Skill 统一管理
...
- **输出边界（统一规则：SVG 优先，PNG 等价）**：每个子项目至少 1 个独立 Quickstart 页面；顶层仅做索引与整体架构。

---

### 5.3 信息抽取原则（统一规则：SVG 优先，PNG 等价）
...
### 5.4 防止过度扫描与噪音（统一规则：SVG 优先，PNG 等价）
...
- 文档中仅引用图资产（SVG 优先，PNG 等价）（见 8.4）

---

## 10. VitePress 生成规范（统一规则：SVG 优先，PNG 等价）
...
- 图资产目录存在，关键图已生成图资产（SVG 优先，PNG 等价）并被文档引用
- 图资产具备 `svg + src` 配对文件
